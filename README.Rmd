...
The results should be exactly the same as if applying the same group-by operations on a data.frame. If not then please [report a bug](https://github.com/xiaodaigh/disk.frame/issues). The current list of functions in a `group_by-summarize` are `min, max, mean, sum, median, length, sd, var`. If a function you like is missing, please make a feature request [here](https://github.com/xiaodaigh/disk.frame/issues).
