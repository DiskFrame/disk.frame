...
setup({
  ...
  data.table::fwrite(df, "tmp_rn_delete.csv")
})

test_that("dplyr::rename", {
  dff = csv_to_disk.frame("tmp_rn_delete.csv", "tmp_rn_delete.df")
  nd = names(dff)
...
teardown({
  fs::file_delete("tmp_rn_delete.csv")
  fs::dir_delete("tmp_rn_delete.df")
})