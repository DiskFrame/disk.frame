---
title: "Benchmarking disk.frame 2: disk.frame beats Dask again! disk.frame beats JuliaDB again! Anyone else wanna challenge?"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Benchmarking disk.frame 2: disk.frame beats Dask! disk.frame beats JuliaDB! Anyone else wanna challenge?}
...
```{r convert, dependson='setup', include=FALSE}
files = list.files("c:/data/perf/", pattern="*.txt", full.names = TRUE)

fz = purrr::map_dfr(files, ~{
  data.frame(file = .x, sz = file.size(.x))
}) %>% arrange(desc(sz)) %>% 
  head(2)
```
