...
arrange(flights.df, desc(dep_delay)) %>% collect
```
...
The `do` function is also supported for performing abitrary computation on each chunk
```{r, dependson='asdiskframe'}
...
collect_fildpllist(flights.df1) # returns number of rows for each data.frame in a list
```
...
In the `disk.frame` framework, sampling a proportion of rows. More sophisticated sampling scheme or method will be implemented in the future
