...
The `do` function is also supported for performing arbitrary computation on each chunk
```{r, dependson='asdiskframe'}
# get the first two rows of each chunk
do(flights.df, head(., 2)) %>% 
...
```{r, dependson='asdiskframe'}
flights.df %>%
  group_by(carrier) %>% # that hard = F to aggregate within each chunk
...
flights.df %>%
  ...
  group_by(carrier) %>% # notice that hard = T needs to be set
  summarize(count = n(), mean_dep_delay = mean(dep_delay, na.rm=T)) %>%  # mean follows normal R rules
  collect
```
...
flights.df2 <- map(flights.df, ~.x[1:10,], lazy = F, outdir = "tmp2", overwrite = T)
