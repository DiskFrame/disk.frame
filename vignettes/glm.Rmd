...
## Logistic regression
Logistic regression is one of the most commonly deployed machine learning (ML) models. It is often used to build binary classification model
...
## Notes
Unfortunately, neither `biglm` nor `speedglm` are managed on open-source platforms, so it's more difficult to contribute to them by making bug fixes and submitting bug reports. So bugs are likely to persists. There is an active effort on `disk.frame` to look for alternatives. Example of avenues to explore include tighter integration with {`keras`}, h2o, or Julia's OnlineStats.jl for model fit purposes.
