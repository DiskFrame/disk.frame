...
csv.to.disk.frame <- function(infile, outpath, ..., mem.prop = 0.1) {
  ...
  # write one row to disk to get an idea of file size
  tf <- tempfile()
  write.csv(file = tf, data[1:1024,] , row.names = FALSE, col.names= FALSE)
  
  # the approximate total file size fpr the first chunk
  tfs <- file.size(tf)/1024 * n
